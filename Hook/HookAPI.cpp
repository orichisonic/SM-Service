...
int CHookAPI::Init()
{
	...
	tagGetMyAPIInfo GetMyAPIInfo;//映射到mydll的 MYAPIINFO *GetMyAPIInfo()
	tagSetServerInfo SetServerInfo;//映射到mydll的SERVERINFO *SetServerInfo()
	APIFUNC old_api, my_api;//映射到要hook的函数
	HMODULE hmod;
	HINSTANCE hMyDll;
	...
	char pInitFile[512];
	DWORD result;
	sprintf(pInitFile,"%s\\RunService.ini",g_szDllPath);	
	result = GetPrivateProfileString("Server","Address","127.0.0.1",dest_ip_addr, 128,pInitFile);
	iPort = GetPrivateProfileInt("Server","Port",10118,pInitFile);

	WriteLog("Addr:%s,Port:%d",dest_ip_addr,iPort);

	if((SetServerInfo=(tagSetServerInfo)GetProcAddress(hMyDll, "SetServerInfo")) == NULL)
	{
		WriteLog("GetProcAddress SetServerInfo of %s failed!", MY_DLL_NAME);
		return false;
	}
	else
	{
		WriteLog("GetProcAddress SetServerInfo of %s succeed!", MY_DLL_NAME);
	}
	SetServerInfo(dest_ip_addr,iPort);
...
int CHookAPI::HookOneAPI(APIINFO *pinfo)
{
	...
	BYTE *papi =(BYTE *)pinfo->old_api + pinfo->start_pos;  // insert call xxxxxxxx from start_pos. 
...
//#endif
	...
	if(!strcmp(pinfo->api_name, "CreateProcessW") || !strcmp(pinfo->api_name, "CreateProcessA") )
	{
		...
		if(pi->hProcess)
		{
			...
			ObtainSeDebugPrivilege();
#endif
			//WriteLog("hook 新进程 %d:%s",pi->hProcess, fname);
